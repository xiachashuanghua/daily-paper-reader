...
tags: ["query:llmagt"]
score: 7.0
evidence: 基于LLM的多智能体框架，用于自动化金属有机框架模拟
tldr: SimMOF是一个基于大语言模型的多智能体框架，旨在自动化金属有机框架（MOF）的端到端模拟流程。针对MOF模拟中对专家经验、参数选择及工具互操作性的高度依赖，该框架能将自然语言指令转化为可执行计划，自动生成输入并协调多个智能体完成模拟与结果分析。SimMOF通过模拟人类研究者的决策过程，显著降低了MOF研究的门槛，为数据驱动的材料科学提供了可扩展的自动化基础。
selection_source: fresh_fetch
motivation: MOF模拟过程复杂，需要专家在工作流构建、参数选择和结构准备等方面进行繁琐的决策。
method: 提出SimMOF框架，利用大语言模型驱动的多智能体系统，将自然语言查询转化为依赖感知的执行计划并自动运行。
result: 案例研究表明，SimMOF能够实现具有认知自主性的自适应工作流，并生成与用户需求高度一致的分析总结。
conclusion: SimMOF为数据驱动的MOF研究提供了一个可扩展的自动化平台，能够有效模拟人类专家的迭代决策行为。
---
