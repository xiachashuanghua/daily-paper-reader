...
## 摘要
随着前沿多模态大语言模型（MLLMs）在数据理解和视觉推理方面的最新进展，LLM 的角色已从被动的“LLM 作为接口”演变为主动的“LLM 作为评判者”，从而实现了与科学数据分析和可视化流程的深度集成。然而，现有的科学可视化智能体仍依赖领域专家为特定数据集提供先验知识，或依赖面向可视化的目标函数通过迭代反馈来引导工作流。这种反应式的、依赖数据的、人机回环（HITL）范式非常耗时，且无法有效扩展到大规模科学数据。在这项工作中，我们提出了 SASAV（用于科学分析与可视化的自主智能体），这是首个无需任何外部提示或 HITL 反馈即可执行科学数据分析并生成具有洞察力的可视化的全自主 AI 智能体。SASAV 是一个多智能体系统，通过我们提出的组件（包括自动化数据剖析、上下文感知知识检索和推理驱动的可视化参数探索）自动编排数据探索工作流，同时支持下游交互式可视化任务。这项工作为未来的 AI for Science 奠定了基础，旨在加速大规模的科学发现与创新。
