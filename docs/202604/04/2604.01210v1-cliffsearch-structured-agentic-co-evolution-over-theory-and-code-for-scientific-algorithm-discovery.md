...
tldr: 针对当前LLM引导的算法发现系统往往只优化代码而忽视科学结构和正确性的问题，本文提出CliffSearch框架。该框架采用智能体演化机制，将理论与代码结合作为结构化产物，通过审稿智能体进行正确性和原创性把关。它将变异分为探索和修正路径，在Transformer超连接演化和优化器发现等任务中，实现了兼顾科学可解释性、正确性与性能指标的算法自动发现。
selection_source: fresh_fetch
motivation: 现有的LLM搜索系统通常只优化代码，缺乏对科学理论结构的表达以及对正确性和原创性的严格筛选。
method: 提出一种智能体演化框架，通过理论与代码协同演化、引入审稿人机制作为选择门控，并将变异分为跨领域探索和证据导向的修正。
result: 在Transformer超连接演化和优化器发现等多个基准测试中，该框架成功发现了具有高性能、高可解释性且符合科学规范的新算法。
conclusion: CliffSearch证明了通过结构化理论引导和严格的质量把关，可以实现比单纯追求候选数量更高效、更具科学价值的算法发现流程。
---
