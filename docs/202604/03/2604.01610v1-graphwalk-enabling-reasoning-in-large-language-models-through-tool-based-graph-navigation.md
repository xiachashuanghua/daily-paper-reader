...
tldr: 针对大规模知识图谱推理中上下文窗口受限的问题，本文提出 GraphWalk 框架。该框架无需训练，通过为大语言模型提供一组最小化的正交图操作工具，使其能够以序列化导航的方式在图结构中进行多步推理。实验表明，GraphWalk 在迷宫遍历和复杂逻辑查询任务中表现优异，显著提升了模型处理大规模图数据的推理能力和透明度。
selection_source: fresh_fetch
motivation: 传统的提示词或检索增强方法难以处理超出上下文窗口限制的企业级大规模知识图谱。
method: 提出一种基于工具的导航框架，通过提供基础图操作接口，引导模型通过序列化步骤在图中进行路径搜索和推理。
result: 在迷宫遍历和合成图逻辑查询任务中，GraphWalk 相比上下文基线取得了显著且持续的性能提升，且模型规模越大优势越明显。
conclusion: 通过工具化导航，LLM 能够有效克服上下文限制，在复杂图结构上实现可验证、透明且高性能的多步推理。
---
