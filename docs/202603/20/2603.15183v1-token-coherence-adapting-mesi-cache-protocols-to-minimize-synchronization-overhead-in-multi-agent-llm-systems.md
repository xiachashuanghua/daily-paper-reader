...
tldr: "针对多智能体LLM系统中全状态广播导致的同步开销爆炸问题，本文借鉴多处理器缓存一致性中的MESI协议，提出了“令牌一致性”系统（ACS）。通过延迟失效机制，将同步成本从三倍乘法复杂度降低到线性复杂度。经TLA+验证及多场景仿真，该方法在保持单写安全的同时，实现了84%至95%的Token节省，并提供了与主流框架集成的参考实现。"
selection_source: fresh_fetch
motivation: 解决多智能体LLM协作中因全量状态广播导致的同步成本随智能体数量、步骤和数据规模呈三倍乘法增长的效率瓶颈。
method: 将硬件MESI缓存一致性协议映射至工件同步，构建工件一致性系统（ACS）并提出令牌一致性定理以实现延迟失效。
result: "在多种工作负载仿真中实现了84.2%至95.0%的Token节省，显著优于理论保守下界，且在极端变动率下仍表现稳健。"
conclusion: 证明了借鉴经典缓存协议可大幅降低多智能体系统的通信开销，为构建高效、可扩展的LLM编排系统提供了形式化验证的路径。
---
