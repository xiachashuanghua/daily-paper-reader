...
tldr: "针对大语言模型在医学多步推理中依赖“捷径学习”的问题，本文提出了ShatterMed-QA基准测试。该基准包含10,558个双语临床问题，通过创新的k-Shattering算法修剪知识图谱中的通用中心节点，强制模型进行深层诊断推理。研究发现，现有模型在处理此类复杂任务时表现大幅下降，而RAG技术能有效缓解这一缺陷。"
selection_source: fresh_fetch
motivation: 现有医学基准测试常因“捷径学习”导致模型仅靠单跳事实召回而非真正的多步临床推理。
method: 提出k-Shattering算法修剪知识图谱中的通用节点，并结合桥接实体掩码和拓扑驱动的负采样构建具有挑战性的多步推理题目。
result: 对21个大模型的评估显示，在排除逻辑捷径后，模型性能显著下降，尤其是领域特定模型表现不佳。
conclusion: ShatterMed-QA有效揭示了当前医学AI在深层推理方面的局限性，并证明了检索增强生成（RAG）在修复推理缺陷中的潜力。
---
